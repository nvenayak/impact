...
class Project(object):
    ...
    def copy_authorized_db(self, experiment_id_list, new_db_name):
        # untested
        experiment_list = self.get_experiment_subset(experiment_id_list)
        init_db(new_db_name)
        for experiment in experiment_list:
            experiment.db_commit(new_db_name)

    def get_experiment_subset(self, experiment_id_list):
        # untested
        experiment_list = []
        for experiment_id in experiment_id_list:
            temp_experiment = Experiment()
            temp_experiment.db_load(db_name, experiment_id)
            experiment_list.append(temp_experiment)
    ...
    #     app.setApplicationName('impact Plotting Interface')
    #
    ...
    #                                  '$\mu$ = ' + '{:.2f}'.format(
    #                                      replicateToPlot.avg.OD.fit_params[1]) + ' $\pm$ ' + '{:.2f}'.format(
    #                                      replicateToPlot.std.OD.fit_params[1]) + ', n=' + str(
